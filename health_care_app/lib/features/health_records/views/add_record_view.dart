...
class AddRecordView extends StatefulWidget {
  ...
  const AddRecordView({super.key, this.record});

  @override
  State<AddRecordView> createState() => _AddRecordViewState();
}

class _AddRecordViewState extends State<AddRecordView> with SingleTickerProviderStateMixin {
  ...
  Future<void> _save() async {
    ...
    // If editing a non-today record, just close — editing is not allowed.
    if (isEditMode && !_isToday(widget.record!.date)) {
  ...
  Widget build(BuildContext context) {
    final isEditMode = widget.record != null;
    final editingAllowed = isEditMode ? _isToday(widget.record!.date) : true;
    final formEnabled = (!_dateExists || isEditMode) && editingAllowed;
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                PrimaryButton(
                  label: isEditMode
                      ? (editingAllowed ? 'Update Record' : 'Close')
                      : 'Save Record',
                  icon: Icons.save,
                  onPressed: _save,
                  isLoading: _isLoading,
                  isEnabled: (formEnabled && !_checkingDate) || (isEditMode && !editingAllowed),
                ),
              ],
            ),
          ),
